...
export async function checkDataIntegrity(): Promise<ValidationResult> {
  ...
  try {
    ...
    const allTaskFiles = await readDirRecursive(".claude", (file) =>
      /\/\d+\.md$/.test(file)
    );
...
export async function checkReferences(): Promise<ValidationResult> {
  ...
  try {
    ...
    const taskFiles = await readDirRecursive(".claude/epics", (file) =>
      /\/\d+\.md$/.test(file)
    );
...
export async function checkFrontmatter(): Promise<ValidationResult> {
  ...
  try {
    ...
    const epicFiles = await readDirRecursive(
      ".claude/epics",
      (file) => file.endsWith(".md")
    );
    const prdFiles = await readDirRecursive(
      ".claude/prds",
      (file) => file.endsWith(".md")
    );
