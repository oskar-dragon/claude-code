...
test("checkDataIntegrity detects missing epic.md files", async () => {
  ...
  expect(
    result.messages.some((m) => m.message.includes("Missing epic.md"))
  ).toBe(true);
});
...
test("checkDataIntegrity detects orphaned task files", async () => {
  ...
  expect(result.warnings).toBeGreaterThan(0);
  expect(
    result.messages.some((m) => m.message.includes("orphaned task files"))
  ).toBe(true);
});
...
test("checkReferences detects broken task dependencies", async () => {
  ...
  expect(result.warnings).toBeGreaterThan(0);
  expect(
    result.messages.some((m) => m.message.includes("references missing task"))
  ).toBe(true);
});
...
test("checkFrontmatter detects missing frontmatter", async () => {
  await mkdir(".claude/epics/test-epic", { recursive: true });
  await writeFile(
    ".claude/epics/test-epic/epic.md",
    "# No frontmatter\n",
    "utf-8"
  );
  ...
  expect(
    result.messages.some((m) => m.message.includes("Missing frontmatter"))
  ).toBe(true);
});
